...
class Submodule(util.IndexObject, Iterable, Traversable):
	...
	def add(cls, repo, name, path, url=None, branch=None, no_checkout=False):
		...
		# fake-repos - we only need the functionality on the branch instance
	...
	def update(self, recursive=False, init=True, to_latest_revision=False, progress=None, 
		...
		try:
			...
			if not dry_run:
				...
				# NOTE: Have to write the repos config file as well, otherwise
	...
	def module(self):
		...
		try:
			...
			if repo != self.repo:
				return repo
			# END handle repos uninitialized
