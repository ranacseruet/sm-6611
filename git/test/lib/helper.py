		...
		def repo_creator(self):
			...
			try:
				try:
					...
					print >> sys.stderr, "Keeping repos after failure: %s" % repo_dir
					repo_dir = None
					raise
			finally:
				os.chdir(prev_cwd)
				...
				# END rm test repos if possible
			# END cleanup
		# END rw repos creator
...
def with_rw_and_rw_remote_repo(working_tree_ref):
	...
	the rw repos has a working tree and was cloned from the remote repository.
...
class TestBase(TestCase):
	...
	- Class level repository which is considered read-only as it is shared among 
	  ...
	  The rorepo is in fact your current project's git repos. If you refer to specific
