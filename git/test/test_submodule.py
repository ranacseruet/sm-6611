...
class TestSubmodule(TestBase):
	...
	def _do_base_tests(self, rwrepo):
		...
		if rwrepo.bare:
			...
			# or we raise
			self.failUnlessRaises(ValueError, Submodule.add, rwrepo, "newsubm", sm.path, "git://someurl/repos.git")
			...
			# END for each repos to reset
			
			# dry run does nothing 
			sm.update(recursive=True, dry_run=True, progress=prog)
			for repo in smods:
				assert repo.head.commit != repo.head.ref.tracking_branch().commit
			# END for each repos to check
			...
			# now delete only the module of the get_git submodule
			assert sm.module_exists()
			...
			# ADD NEW SUBMODULE
			###################
			# add a simple remote repos - trailing slashes are no problem
			smid = "newsub"
			osmid = "othersub"
			...
			# REMOVE 'EM ALL
			################
			# if a submodule's repos has no remotes, it can't be added without an explicit url
			osmod = osm.module()
	...
	def test_root_module(self, rwrepo):
		...
		csmadded = rwrepo.index.commit("Added submodule").hexsha	# make sure we don't keep the repos reference
		nsm.set_parent_commit(csmadded)
		assert nsm.module_exists()
		# in our case, the module should not exist, which happens if we update a parent
		# repos and a new submodule comes into life
