...
class TestRemote(TestBase):
    ...
    def _do_test_fetch_result(self, results, remote):
        # self._print_fetchhead(remote.repos)
    ...
    def _commit_random_file(self, repo):
        #Create a file with a random name and random data and commit it to  repos.
    ...
    def _do_test_fetch(self,remote, rw_repo, remote_repo):
        # specialized fetch testing to de-clutter the get_git test
        ...
        # index must be false as remote is a bare repos
        ...
        # Create a new UN-shared repos and fetch into it after we pushed a change
        # to the shared repos
        other_repo_dir = tempfile.mktemp("other_repo")
        # must clone with a local path for the repos implementation not to freak out
